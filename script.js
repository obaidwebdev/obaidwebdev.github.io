(function createSpaceLoader() {
  ...
  function initSpacePhysics(container) {
    ...
    const material = new THREE.PointsMaterial({ size: 0.05, color: 0x008BFD, transparent: true, opacity: 0.7 });
...
(function initInteractiveSpace() {
  ...
  script.onload = () => {
    ...
    const material = new THREE.PointsMaterial({ size: 0.08, color: 0x008BFD });
    const particles = new THREE.Points(geometry, material);
    scene.add(particles);
...
(() => {
  ...
  function drawTrail() {
    ...
    g.addColorStop(0, "rgba(0,139,253,0)");
    g.addColorStop(1, "rgba(255,255,255,.85)");

    ctx.strokeStyle = g;
    ctx.lineWidth = 3;
    ctx.shadowBlur = 18;
    ctx.shadowColor = "#008BFD";
    ctx.stroke();
    ctx.shadowBlur = 0;
  ...
  function drawParticles() {
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      ctx.beginPath();
      ctx.fillStyle = `rgba(0,139,253,${p.life})`;
      ctx.shadowBlur = 12;
      ctx.shadowColor = "#008BFD";
  ...
  function drawSparks() {
    for (let i = sparks.length - 1; i >= 0; i--) {
      ...
      ctx.beginPath();
      ctx.strokeStyle = `rgba(0,139,253,${s.life})`;
      ctx.lineWidth = 2;
      ctx.shadowBlur = 18;
      ctx.shadowColor = "#008BFD";
